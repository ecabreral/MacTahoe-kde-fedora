#!/usr/bin/env bash
# Provision a CachyOS / Arch KDE Plasma 6 machine with the MacTahoe-kde theme.
...
install_kvantum() {
  if pacman -Q kvantum >/dev/null 2>&1; then
    say "kvantum ya instalado"
    return 0
  fi
  if [[ "${OFFLINE}" -eq 1 ]]; then
    warn "Modo offline: sigo sin kvantum (luego: sudo pacman -S kvantum)"
    return 0
  fi
  say "Instalando kvantum desde los repos (sudo)"
  run_root pacman -S --noconfirm --needed kvantum || warn "kvantum no se pudo instalar; el resto del tema se aplica igual"
}
...
if [[ "${ENABLE_SSH}" -eq 1 ]]; then
  ...
  run_root pacman -S --noconfirm --needed openssh
  run_root systemctl enable --now sshd
  warn "Si hay firewall (nftables de CachyOS) activo, permite el puerto 22"
fi
